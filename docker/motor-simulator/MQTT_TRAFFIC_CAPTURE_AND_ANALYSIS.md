...
|----------------|-----------------|
| ![**Packet 1** (first captured publish)](wireshark_screenshots/Screenshot_2026-06-03_175019.png.png) | Shows `MotorOut: true`. Capture started after script had already run for a few seconds. |
| ![**Packet 41** (first toggle after publish began)](wireshark_screenshots/Screenshot_2026-06-03_175030.png) | `MotorOut: false`. The time gap between Packet 1 and Packet 41 is ~9 seconds, not exactly 10, because the first packet was not the very first publish. |
| ![**Packet 85** (second toggle)](wireshark_screenshots/Screenshot_2026-06-03_175042.png) | `MotorOut: true`. Time difference between Packet 41 and Packet 85 is 10.0099 seconds, the correct periodic alternation. |
| ![**MQTT/TCP ACK alternation**](wireshark_screenshots/Screenshot_2026-06-03_175703.png) | Shows that after each MQTT publish packet, the broker responds with a pure TCP ACK (Len:0). This is normal TCP behaviour and confirms reliable delivery. |
